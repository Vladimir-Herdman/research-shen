...
class MyCamerasPreviewFrame(customtkinter.CTkFrame):
    ...
    def set_images(self, a: str, b: str, c: str, d: str):
        img_a = Image.open(a)
...
class MyLoadingFrame(customtkinter.CTkFrame):
    ...
    def update_loading(self):
        #TODO: make it check if Cameras.cap1-4.isOpened() and then initiate the green screen or red after a minute
        while (self.loading):
...
class App(customtkinter.CTk):
    ...
    def connect_cams(self):
        try:
            self.cam1 = Cameras("Camera1", 0) #TODO: replace with proper numbers
            self.cam2 = Cameras("Camera2", 0) #
            self.cam3 = Cameras("Camera3", 0) #
            self.cam4 = Cameras("Camera4", 0) #
            time.sleep(3)
            Cameras.check_cams()
        except Exception:
            pass
    ...
    def preview_cameras_func(self):
        if ((hasattr(self, "cam1") and hasattr(self, "cam2") and hasattr(self, "cam3") and hasattr(self, "cam4")) and (self.cam1.cap.isOpened() and self.cam2.cap.isOpened() and self.cam3.cap.isOpened() and self.cam4.cap.isOpened())):
            ...
            self.preview_cameras.set_images(image_path_a, image_path_b, image_path_c, image_path_d)
